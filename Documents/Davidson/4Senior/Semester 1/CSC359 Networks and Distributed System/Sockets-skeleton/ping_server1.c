...
// Ping-pong version 1: one client ever handled
...
int main(int argc, char **argv) {
	...
	while(1) {
		...
		int nread = receive_string(client_socket, buffer, BUFFER_SIZE - 1);
		...
		printf("Type your message to the client: ");
		fgets(buffer, BUFFER_SIZE, stdin);

		flush_buffer(client_socket, buffer, strlen(buffer) + 1);
	}
